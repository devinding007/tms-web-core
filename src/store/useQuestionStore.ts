...
export const useQuestionStore = defineStore('question', {
  state: () => ({
    items: [...SAMPLE_QUESTIONS] as Question[],
  }),
  getters: {
    byId: (s) => (id: string) => s.items.find((v) => v.問題ＩＤ === id),
  },
  actions: {
    setAll(rows: Question[]) {
      this.items = rows;
    },
    upsert(row: Question) {
      const i = this.items.findIndex((x) => x.問題ＩＤ === row.問題ＩＤ);
      if (i >= 0) this.items.splice(i, 1, row);
      else this.items.push(row);
    },
    remove(id: string) {
      const i = this.items.findIndex((x) => x.問題ＩＤ === id);
